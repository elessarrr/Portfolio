...
*   **ASN 429 rate-limiting silently drops incidents on big catch-up runs:** a large backfill (~100+ new detail fetches back-to-back) trips aviation-safety.net's limiter (314×429); `get_soup` used to swallow 429 → skipped models / null-narrative imports. Fix: `get_soup()` exponential backoff respecting `Retry-After`. See `docs/solutions/integration-issues/asn-429-rate-limit-backoff.md`.
