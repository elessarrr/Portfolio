...
- [x] 2.0 Database Design & Models
  ...
  - [x] 2.2 Add Incident table to `app/models.py` (id, aircraft_id FK, date, operator, location, fatalities, description, asn_url, incident_type)
  - [x] 2.3 Add Request table to `app/models.py` for user feedback (id, aircraft_model, user_email nullable, created_at)
  - [x] 2.4 Initialize Flask-Migrate and create initial migration (`flask db init && flask db migrate -m "Initial schema"`)
  - [x] 2.5 Apply migration to create SQLite database (`flask db upgrade`)
...
- [ ] 3.0 Data Collection & Import (UPDATED STRATEGY)
  ...
  - [ ] 3.4 Write ASN scraper in `scripts/scrape_airbus.py`. **Strategy:** Start at ASN "Accidents by type" index. Filter for Airbus models (A220, A300, A310, A319/320/321, A330, A340, A350, A380). **Crucial:** Follow incident date hyperlinks to scrape "Fatalities" and "Narrative" from details page.
