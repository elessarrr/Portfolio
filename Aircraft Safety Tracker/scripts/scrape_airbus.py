...
TYPE_INDEX_URL = "https://aviation-safety.net/asndb/types/A"
TARGET_MODELS = ["Airbus"]  # We want all Airbus models
...
def get_model_links(client):
    """Scrape the type index page to find links for target Airbus models."""
    ...
    # Look for links starting with /asndb/type/ and text starting with "Airbus"
    for link in soup.find_all('a', href=True):
        text = link.get_text().strip()
        href = link['href']
        
        if "/asndb/type/" in href and text.startswith("Airbus"):
...
def main():
    ...
    # Check if file exists to resume? (Simplification: just overwrite for now)
