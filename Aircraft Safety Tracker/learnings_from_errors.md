...
- Error: New FAA SDR importer tests failed with `TypeError: Can't instantiate abstract class FAASDRImporter with abstract methods parse, upsert`.
- Cause: The importer inherits abstract methods from `DataSourceImporter`, so unit tests cannot instantiate it directly.
- Fix: Introduced a minimal test-only subclass that implements `parse()` and `upsert()` no-op behavior, then exercised fetch/filter logic through that concrete subclass.
- Prevention: For unit tests on abstract importer behavior, always instantiate a minimal concrete subclass so tests target behavior under test instead of ABC enforcement.

## 2026-04-05 (4)

- Error: Cache behavior test for report analysis expected first response `cached=False`, but assertion read `True`.
- Cause: Cached response path mutates the same dict instance stored in cache, and the first-response reference pointed to the same object later updated to `cached=True`.
- Fix: Captured the first cached state before the second request and asserted on the snapshot rather than the mutated shared object.
- Prevention: In cache tests, avoid asserting mutable dict fields through reused object references across calls; snapshot values or deep-copy payloads before second mutation.
