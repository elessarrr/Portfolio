...
- `app/static/css/styles.css` - Includes `.line-clamp-3`, `.dashboard-charts`, and global HTMX indicator bar styling (`#htmx-global-indicator`).
- `app/static/js/main.js` - Handles read-more toggles and now emits visual global error toasts for `htmx:responseError` and `htmx:sendError`.
- `app/templates/incidents_database.html` - Dashboard chart wrapper and "Apply Filters" button now use global HTMX indicator for visible update feedback.
...
- `app/services/report_analyzer.py` - DeepSeek and Gemini analyzer adapters now use explicit request timeout values to prevent hanging model calls.
- `app/services/gemini.py` - Gemini service now supports explicit timeout via `request_options` in `generate_content`.
...
- `app/templates/base.html` - Includes both global loading indicator (`#htmx-global-indicator`) and hidden toast host (`#toast-container`) for feedback UX.
- `app/templates/index.html` - Search input now points `hx-indicator` to the global HTMX indicator for consistent loading feedback.
...
- [x] 3.0 AI Summary Backend Fixes
...
- [ ] 4.0 Global Feedback States
  ...
  - [x] 4.4 Add a hidden `#toast-container` to `base.html`.
  - [x] 4.5 Write a small JS listener in `main.js` that listens for `htmx:responseError` and `htmx:sendError` events, and triggers a visual error toast in the `#toast-container`.
