"""NTSB make_model → aircraft page mapping (PRD 0006.3 FR-16 / FR-17)."""
...
VALID_ACTIONS = frozenset({"map_to_existing", "create_approved"})


@dataclass(frozen=True)
class NtsbMappingEntry:
    ntsb_make_model: str
...
class NtsbMakeModelMapping:
    """Pre-import gate: NTSB make_model string → catalog Aircraft row."""

    def __init__(self, entries: Dict[str, NtsbMappingEntry]):
        self._entries = entries

    @classmethod
    def load(cls, path: Path | str) -> "NtsbMakeModelMapping":
        path = Path(path)
        entries: Dict[str, NtsbMappingEntry] = {}
        with path.open() as f:
            for line_no, line in enumerate(f, start=1):
                ...
                if entry.ntsb_make_model in entries:
                    raise ValueError(
                        f"{path}:{line_no}: duplicate ntsb_make_model {entry.ntsb_make_model!r}"
                    )
                entries[entry.ntsb_make_model] = entry
        if not entries:
            raise ValueError(f"{path}: no mapping entries")
        return cls(entries)

    def get(self, ntsb_make_model: str) -> Optional[NtsbMappingEntry]:
        return self._entries.get(ntsb_make_model)

    def __len__(self) -> int:
        return len(self._entries)

    def resolve_aircraft_id(self, ntsb_make_model: str) -> Optional[int]:
        """Return aircraft_id for mapped string; None if unmapped or unresolved."""
        entry = self.get(ntsb_make_model)
        if entry is None:
            return None
        ...
        logger.warning("unknown mapping action %r for %r", entry.action, ntsb_make_model)
        return None


def load_ntsb_make_model_mapping(path: Path | str) -> NtsbMakeModelMapping:
    return NtsbMakeModelMapping.load(path)


def resolve_aircraft_id_from_ntsb_mapping(
    make_model: str, mapping: NtsbMakeModelMapping
) -> Optional[int]:
    return mapping.resolve_aircraft_id(make_model)


def iter_create_approved_targets(
    mapping: NtsbMakeModelMapping,
) -> Dict[str, NtsbMappingEntry]:
    """Distinct canonical_model_name → representative create_approved entry."""
    targets: Dict[str, NtsbMappingEntry] = {}
...
def bootstrap_create_approved_pages(
    ...
    *,
    ...
    """Create empty Aircraft rows for all distinct create_approved mapping targets (FR-20.0)."""
...
def _parse_entry(raw: dict, path: Path, line_no: int) -> NtsbMappingEntry:
    required = ("ntsb_make_model", "canonical_model_name", "action")
    for key in required:
        if key not in raw:
    ...
    if action == "create_approved" and not manufacturer:
        raise ValueError(
            f"{path}:{line_no}: create_approved requires manufacturer for {raw['ntsb_make_model']!r}"
        )
    return NtsbMappingEntry(
        ntsb_make_model=raw["ntsb_make_model"],
...
def _get_or_create_approved(entry: NtsbMappingEntry) -> Optional[int]:
