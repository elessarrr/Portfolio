...
- `app/routes.py` - Removes legacy 1985 fallbacks and now caps aircraft detail/list incident queries to `limit(50)` to protect page-load performance.
- `tests/test_routes.py` - Adds regression coverage for `date_from`, detail/list `limit(50)`, and null-date sorting safety across detail/list/export routes.
...
- [x] 2.0 Remove Hardcoded Date Filters from Backend Routes
