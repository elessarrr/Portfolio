...
- [ ] 3.0 Incidents Database Page UI & Infinite Scroll
  ...
  - [ ] 3.3 Implement HTMX infinite scroll on the last element of the list (`hx-get="/incidents/page?page=2" hx-trigger="revealed" hx-swap="afterend"`).
- [ ] 4.0 Filtering & Search Capabilities
  - [ ] 4.1 Add a filter sidebar to `incidents_database.html` with inputs for: Year Range, Aircraft Manufacturer/Model, Incident Severity, and Geographical Location.
  - [ ] 4.2 Wire the filter form using HTMX (`hx-get="/incidents" hx-target="#incident-container" hx-trigger="change"`) so changing a filter resets the list to page 1 and applies the new criteria.
  - [ ] 4.3 Ensure the backend pagination endpoint (`/incidents/page`) respects the currently active filters passed in the query string.
