...
- 🟡 In Progress — 29% complete (4/14 tasks checked)

## Tasks

- [ ] 1.0 Remove Variant Comparison UI Component
...
- [ ] 2.0 Remove Hardcoded Date Filters from Backend Routes
  - [ ] 2.1 Open `app/routes.py`.
  - [ ] 2.2 In the `aircraft_details` route, locate the `query.filter(Incident.date >= datetime(1985, 1, 1).date())` condition applied when `date_from` is absent. Remove this hardcoded fallback.
  - [ ] 2.3 In the `get_incidents` route, locate and remove the identical `1985` date filter logic.
  - [ ] 2.4 In the `export_incidents_csv` route, locate and remove the identical `1985` date filter logic.
  - [ ] 2.5 Ensure that `apply_incident_filters` (or equivalent logic) still correctly applies a user-defined "From Date" if provided via the request arguments.
